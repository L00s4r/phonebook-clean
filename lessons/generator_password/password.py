...
class PasswordGenerator(QWidget):
    ...
    def initUI(self):
        ...
        self.uppercase_cb = QCheckBox('Uppercase strings (A-Z)')
        self.uppercase_cb.setChecked(False)
        self.lowercase_cb = QCheckBox('lowercase strings (a-z)')
        self.lowercase_cb.setChecked(False)
        self.digits_cb = QCheckBox('Integers (0-9)')
        ...
        self.generate_btn = GlowButton('Generate')
        self.copy_btn = GlowButton('Copy')
        ...
        content_layout.addWidget(self.length_label)
        content_layout.addWidget(self.length_slider)
        content_layout.addWidget(self.uppercase_cb)
        content_layout.addWidget(self.lowercase_cb)
        content_layout.addWidget(self.digits_cb)
        content_layout.addWidget(self.symbols_cb)
    ...
    def generate_password(self):
        ...
        if self.symbols_cb.isChecked():
            chars += "!@#$%^&*()_+-=[]{};:,.<>?/\\|"
        if not chars:
            QMessageBox.critical(self, "Critical", "Please, choose at least one type of symbols")
            return
    ...
    def copy_password(self):
        user_input = self.password_edit.text().strip()
        if not user_input:
            QMessageBox.critical(self, "Critical", "First, genrate the password!")
            return
        else:
            pyperclip.copy(user_input)
            QMessageBox.information(self, "Copied", "Succesful")
